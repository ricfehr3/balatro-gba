...
bool joker_object_score(JokerObject* joker_object,
                        ...
                        bool* retrigger)
{
    // If the joker has already been processed, return false
    if(joker_object->joker->processed == true)
        return false;
    ...
    if(memcmp(&joker_effect, &(JokerEffect){ 0 }, sizeof(JokerEffect)) != 0)
    {
        ...
        *money += joker_effect.money;
        ...
        int cursorPosX = fx2int(joker_object->sprite_object->x) + 8;
