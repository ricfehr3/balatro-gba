...
int game_speed = 1;    // BY DEFAULT IS ON 1, but if changed to 2, should speed up all (or most) of the game aspects that should be sped up by speed, as in the original game.
...
{
    if (index < 0 || index > hand_top || hand_state != HAND_SELECT)
        return;
    selection_x = index;
...
static void game_playing_process_input_and_state()
{
    ...
    else if (play_state == PLAY_ENDING)
    {
        if (mult > 0)
        {
            ...
            lerped_temp_score = (float)(1.0f / game_speed) * int2fx(temp_score);
...
static void discarded_jokers_update_loop()
{
    ...
    for (int i = list_get_size(discarded_jokers) - 1; i >= 0; i--)
    {
        ...
        joker_object_update(joker_object);
        if (joker_object->sprite_object->x == joker_object->sprite_object->tx && joker_object->sprite_object->y == joker_object->sprite_object->ty)
        {
