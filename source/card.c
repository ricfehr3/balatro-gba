...
void card_object_destroy(CardObject** card_object)
{
    ...
    // card_destroy(&(*card_object)->card); // In practice, this is unnecessary because the card will be inserted into
    // the discard pile and then back into the deck. If you need to destroy the card, you can do it manually before
...
void card_object_set_sprite(CardObject* card_object, int layer)
{
    ...
    memcpy32(&tile_mem[4][tile_index],
             &deck_gfxTiles[card_sprite_lut[card_object->card->suit][card_object->card->rank] * TILE_SIZE],
             TILE_SIZE * CARD_SPRITE_OFFSET);
    sprite_object_set_sprite(
        card_object->sprite_object,
